...
export const _PersonalSchema = z.object({
    ...
    endOfPlan: z.coerce.number({
        required_error: "End of plan is required", // Custom error message if the field is missing
        invalid_type_error: "End of plan must be a number", // Custom error message if the type is wrong
      })
    .int(),
  });
  export const PersonalSchema = _PersonalSchema.required().superRefine((data, ctx) => {
  ...
  export const _AssetSchema = z.object({
  ...
  export const AssetSchema = _AssetSchema.required({name:true,amount:true});
  ...
  export const LifeSchema = _LifeSchema.required({name:true,amount:true,from:true,to:true});
